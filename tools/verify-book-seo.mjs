...
if (mode === '--source') {
  ...
  if (/className="(?:miniBrand|coverChildCharacter)"[^>]*alt=""/.test(source)) errors.push('key homepage images must have descriptive alt text');
  if (/bookBootLang[\s\S]{0,120}<img[^>]*alt=""/.test(html)) errors.push('boot language image must have descriptive alt text');
} else if (mode === '--dist') {
  for (const [language, copy] of Object.entries(locales)) {
    ...
    if (/bookBootLang[\s\S]{0,120}<img[^>]*alt=""/.test(html)) errors.push(`${language}: built boot language image has empty alt text`);
  }
} else {
